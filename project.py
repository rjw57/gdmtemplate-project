# This file is a modified form of the example configuration available at
# https://github.com/GoogleCloudPlatform/deploymentmanager-samples/.
#
# Original copyright information reproduced below:
#
...
def GenerateConfig(context):
    ...
    if context.properties.get('bucket-export-settings'):
        ...
        if context.properties['bucket-export-settings'].get('create-bucket'):
            ...
            resources.append({
                ...
                'metadata': {
                    'dependsOn': [
                        project_id, ApiResourceName(project_id, 'storage-component.googleapis.com')
                    ]
                }
            })
    ...
    if context.properties.get('shared_vpc_service_of'):
        resources.append({
            'name': (
                project_id + '-xpn-service-' + context.properties['shared_vpc_service_of']
            ),
...
def IsProjectParentValid(properties):
    """
    A helper function to validate that the project is either under a folder or under an
    organization and not both

    """
    if 'organization-id' not in properties and 'parent-folder-id' not in properties:
        return False
